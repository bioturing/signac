...
struct data_converter<
    ...
    inline void process_result(std::vector<T>& vec, char *rdata, size_t total_len, size_t el_len) {}
...
struct data_converter<std::vector<std::string>, void> {
    ...
    inline void split(std::vector<std::string> &result, std::string str, std::size_t line_sz )
    {
        if(std::all_of( std::begin(str), std::end(str), [] ( char c ) {
    ...
    inline void process_result(std::vector<std::string>& vec, char *rdata, size_t total_len, size_t el_len) {
        (void)vec;

        size_t totalbyte = total_len * el_len;
        std::string dataString(rdata, totalbyte);

        vec.resize(total_len);
        for(size_t i=0,j=0; i< totalbyte; i+=el_len, ++j)
        {
            vec[j] = dataString.substr(i, el_len);
        }
    }
